...
function AkamaiPurge(urls, options, callback) {
  ...
  mapper = function(urlSet, callback) {
    new AkamaiPurgeRequest(urlSet, options, function(err, response) {
      if (err) {
        this.emit('error', err, response, urlSet, options);
        callback(err, response);
        return;
      }

      this.emit('request-success', response, urlSet, options);
